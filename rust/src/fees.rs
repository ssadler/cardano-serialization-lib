...
#[wasm_bindgen]
pub fn min_fee(tx: &Transaction, linear_fee: &LinearFee, mem_price: f64, step_price: f64) -> Result<Coin, JsError> {
    ...
    if let Some(redeemers) = tx.witness_set().redeemers() {
        let tot_ex = redeemers.tot_ex_units()?;
        let script_fee = to_bignum((mem_price * (from_bignum(&tot_ex.mem()) as f64) + step_price * (from_bignum(&tot_ex.steps()) as f64)).ceil() as u64);
...
mod tests {
    ...
    fn tx_simple_utxo() { // # Vector #1: simple transaction
        ...
        assert_eq!(
            min_fee(&signed_tx, &linear_fee, 0.0, 0.0).unwrap().to_str(),
